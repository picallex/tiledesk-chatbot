...
class DirPicallexSfActivity {
  ...
  execute(directive, callback) {
    winston.verbose("Execute PicallexSfActivity directive");
    let action;
    if (directive.action) {
      action = directive.action;
    } else {
      this.logger.error("Incorrect action for ", directive.name, directive);
      winston.warn("DirPicallexSfActivity Incorrect directive: ", directive);
      callback();
      return;
    }
    this.go(action, (stop) => {
      this.logger.native("[PicallEx SfActivity] Executed");
      callback(stop);
    });
  }

  async go(action, callback) {
    winston.debug("(DirPicallexSfActivity) Action: ", action);
    if (!this.tdcache) {
      winston.error("(DirPicallexSfActivity) Error: tdcache is mandatory");
      callback();
      return;
    }
    ...
    if (!apiKey) {
      this.logger.error("[PicallEx SfActivity] API key not found in integrations");
    ...
    // Build request body
    let body = {
      preLeadId: filler.fill(action.preLeadId, requestVariables),
      entityType: action.entityType,
      activityType: action.activityType,
      subject: filler.fill(action.subject, requestVariables),
      description: filler.fill(action.description, requestVariables),
      scheduleAppointment: false
    };

    winston.debug("(DirPicallexSfActivity) body: ", body);

    const url = PICALLEX_ENDPOINT + "/v1/api/schedules/salesforce/activity";
    ...
    winston.debug("(DirPicallexSfActivity) HttpRequest: ", HTTPREQUEST);

    this.#myrequest(HTTPREQUEST, async (err, res) => {
      if (err) {
        winston.error("(DirPicallexSfActivity) err: ", err);
        let status = res ? res.status : 500;
        let errorMsg = res ? res.error : (err.message || "Unknown error");
        this.logger.error("[PicallEx SfActivity] API error: ", errorMsg);
...
module.exports = { DirPicallexSfActivity };
