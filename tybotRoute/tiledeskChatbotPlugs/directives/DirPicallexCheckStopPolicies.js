...
class DirPicallexCheckStopPolicies {
  ...
  async go(action, callback) {
    ...
    if (leadId === null) {
      this.logger.error("[PicallEx CheckStopPolicies] lead id not found in the request context");
      let errorMsg = "PicallEx CheckStopPolicies Error: lead id is missing or not numeric";
      await this.#assignAttributes(action, null, errorMsg);
      if (falseIntent) {
        await this.#executeCondition(false, trueIntent, trueIntentAttributes, falseIntent, falseIntentAttributes);
        callback(true);
  ...
  #myrequest(options, callback) {
    ...
    guardedAxios(axios_options, "DirPicallexCheckStopPolicies")
      .then((res) => {
        if (callback) {
          callback(null, res);
        }
